...
train=window(sample, end=1397)
test=window(sample, start=1398, end=1440)
...
#fitting and goodness of fit
MAPE(train,bestHoltlinear_model$fitted[,1])
...
#forecast
bestHoltlinear_model_forecast=forecast(bestHoltlinear_model,h=43)
autoplot(bestHoltlinear_model_forecast, main="Holt's linear(1,0.002396703)-original series", 
         ylab='Energy',xlim=c(1000,1500))+autolayer(test, series='actual series')
autoplot(bestHoltlinear_model_forecast, main="Holt's linear(1,0.01253426)-log series", 
         ylab='Energy')+autolayer(test, series='actual series')

MAPE(test,bestHoltlinear_model_forecast$mean)
