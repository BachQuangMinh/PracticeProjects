...
GARCHfit=ugarchfit(data=sample, spec=spec, solver='solnp', out.sample = 43)

GARCHfit

MAPE(sample[1:1440],GARCHfit@fit$fitted.values)

plot.ts(sample[1:1397],type="l",xlab="Time point",ylab="Log energy(Wh)",main="GARCH(1,1)-ARMA(1,1)",col='red')
...
GARCHforecast=ugarchforecast(fitORspec=GARCHfit, n.ahead=43, n.roll=43)
MAPE(sample[1398:1440],GARCHforecast@forecast$seriesFor[,1])
show(GARCHforecast)
plot(GARCHforecast)
