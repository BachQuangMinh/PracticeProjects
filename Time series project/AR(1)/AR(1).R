...
#fit AR(1)
n=length(logsample)
logsamplelag1=c(NA,logsample[1:n-1])
ar1_model=lm(logsample~logsamplelag1,na.action=na.exclude)
summary(ar1_model)
MAPE=mape(logsample[2:n],ar1_model$fitted.values)
AIC=AIC(ar1_model)

plot.ts(logsample,type="l",xlab="Time point",ylab="Log energy(Wh)",main="AR1 model",col='red')
lines(ar1_model$fitted.values,lty=1,col="blue")
grid()
...
for (index in 1:k_step){
  prediction_values[index,2] = ar1_model$coefficients[1] + ar1_model$coefficients[2]*input
